(function(){
  ...
  function matches(item,words){var hay=norm([item.title,item.summary,item.category,item.category_slug,item.topic_slug,item.topic_label,item.date,item.date_iso,item.source_name,Array.isArray(item.tags)?item.tags.join(' '):''].join(' '));return words.every(function(w){return hay.indexOf(w)>-1;});}
  function country(c){return c==='USA'?'US Amerika':c==='Germany'?'DE Almanya':'TR Türkiye';}
  function card(item){var title=item.title||'Haber',href=item.url||item.canonical_url||'#',img=item.image_url||(item.image&&item.image.url)||'',topic=item.topic_slug||item.category_slug||norm(item.category||'gundem').replace(/\s+/g,'-'),a,b,m,h,l,f,r,w,im;
    ...
    b=document.createElement('div');b.className='card-body';m=document.createElement('div');m.className='card-meta';text(m,'span','card-category',country(item.country));text(m,'span','card-topic',item.topic_label||item.category||'Gündem');text(m,'time','card-date',item.date||item.date_iso||'');b.appendChild(m);
